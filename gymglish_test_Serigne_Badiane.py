...
def count_occurrences_in_text(word, text):
    ...
    bad_chars = {ord(';'): None, ord('!'): None, ord('*'): None, ord('?'): None, ord(','): None,
                 ord('('): None, ord(')'): None, ord('\"'): None, ord('/'): None, ord('"'): None,
                 ord('<'): None, ord('>'): None, ord('«'): None, ord('»'): None, ord('\u201c'): None,
                 ord('\u201d'): None, ord('_'): None}

    word = word.translate(bad_chars).replace("'''",'').replace("''",'').replace(':', ' : ').replace('.', ' .').lower()
    text = text.translate(bad_chars).replace("'''",'').replace("''",'').replace(':', ' : ').replace('.', ' .').lower()
    ...
    if len(word_split) == 1:
        ...
        for i in range(len(text.split()) - len(word_split) + 1):
            current_sequence = ' '.join(words_in_text[i:i + len(word_split)])
